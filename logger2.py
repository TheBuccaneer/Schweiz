...
def main():
    ...
    # Output-Datei
    outfile = 'data/raw/pilot_full_improved.csv'
    ...
    with open(outfile, mode='w', newline='') as f:
        ...
        writer.writerow([
            ...
            'energy_j',
            'time_s',
            'gpu_kernel_time_s',  # Additional field for pure GPU kernel time
            'cpu_model',
            'gpu_model',
            'driver_version',
            'rapl_available',
        ...
        for workload in workloads:
            for size in sizes:
                ...
                if CUPY_AVAILABLE:
                    print(f"Warming up GPU for {workload} {size}x{size}...")
                    A_gpu = cp.array(A)
                    B_gpu = cp.array(B)
                ...
                for device in ['CPU', 'GPU']:
                    for rep in range(1, repeats + 1):
                        ...
                        if device == 'GPU':
                            ...
                            # Falls Energy Counter nicht verfÃ¼gbar, nutze Power-Sampling
                            if e_before is None and gpu_handle:
                                # Sammle Power-Samples wÃ¤hrend der AusfÃ¼hrung
                                gpu_power_samples = []
                        else:
                        ...
                        if device == 'GPU' and CUPY_AVAILABLE:
                            ...
                            timing_method = 'CPU_fallback'

                            # Fallback zu CPU mit improved timing:
                            t_start = now()
                            C = A.dot(B)
                            t_end = now()
                            time_elapsed = t_end - t_start
                            ...
                            # CPU execution with improved timing
                            timing_method = 'CPU_perf_counter'
                        ...
                        writer.writerow([
                            ...
                            rapl_available,
                            timing_method,
                            energy_method if device == 'GPU' else 'rapl',  # Track energy measurement method
                            notes
                        ])
                        f.flush()
                        ...
                        # Progress feedback
                        if rep % 5 == 0 or rep == 1:
                            print(f"Completed {workload} {size}x{size} {device} run {rep}/{repeats}")

    shutdown_nvml()
    print(f"Done. Improved pilot data in {outfile}")
