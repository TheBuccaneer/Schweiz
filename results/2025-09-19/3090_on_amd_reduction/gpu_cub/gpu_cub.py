...
def run_cub_baseline(x, out, args):
    ...
    try:
        ...
        if energy_match and power_match:
            try:
                ...
                power_w = float(power_match.group(2))
...
def main():
    parser = argparse.ArgumentParser(
        description='CUB C++ Binary Reduction Benchmark (External CUB DeviceReduce::Sum)'
    )
    ...
    parser.add_argument('--cub-bin', type=str, required=True,
                        help='Path to external C++ binary with real cub::DeviceReduce::Sum')
    ...
    print(f"Implementation: CUB C++ Binary")
    print(f"CUB Binary: {args.cub_bin}")
    ...
    start_event.record()
    final_result = run_cub_baseline(x_buf_final, out_buf_final, args)
    ...
    if not validation_ok:
        ...
        print("   This may be normal for different summation order.")
    else:
        print("✓ Validation passed")

    # CSV output - same structure as original but new filename
    outfile = 'data/raw/reduction_benchmark_gpu_cub.csv'
    ...
    with open(outfile, 'a', newline='') as f:
        ...
        writer.writerow([
            'GPU', 'cub_cpp', 'float32', N, args.passes,
