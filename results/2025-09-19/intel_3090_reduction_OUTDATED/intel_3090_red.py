...
def run_cub_baseline(x, out, args=None):
    ...
    try:
        # Call external CUB binary with passes support
        cmd = [
            args.cub_bin,
            f"--dtype=float32",
            f"--n={x.size}",
        ...
        # Parse both sum and kernel_ms from stdout
        stdout = result.stdout.strip()
        m = re.search(r"^energy_J=([^\s]+)\s+power_W=([^\s]+)", stdout, re.MULTILINE)
        if not hasattr(run_cub_baseline, '_timing'): run_cub_baseline._timing = {}
        run_cub_baseline._timing['energy_J'] = float(m.group(1)); run_cub_baseline._timing['power_W'] = float(m.group(2))

        # Extract sum value
        sum_match = re.search(r"^sum=([^\n]+)", stdout, re.MULTILINE)
        if not sum_match:
            raise RuntimeError(f"CUB binary did not return expected 'sum=' format. Output: {stdout}")
        sum_val = float(sum_match.group(1))

        # Extract kernel timing
        km_match = re.search(r"^kernel_ms=([^\n]+)", stdout, re.MULTILINE)
        if not km_match:
            raise RuntimeError(f"CUB binary did not return expected 'kernel_ms=' format. Output: {stdout}")
        kernel_ms = float(km_match.group(1))

        # Extract energy measurements (if available)
        energy_match = re.search(r"^energy_J=([^\n]+)", stdout, re.MULTILINE)
        power_match = re.search(r"^power_W=([^\n]+)", stdout, re.MULTILINE)

        energy_j = None
        power_w = None

        if energy_match and power_match:
            try:
                energy_j = float(energy_match.group(1))
                power_w = float(power_match.group(1))
                # Validate that values are finite (not NaN)
                if not (np.isfinite(energy_j) and np.isfinite(power_w)):
                    energy_j = power_w = None
        ...
        # Store timing and energy info for later retrieval
        if not hasattr(run_cub_baseline, '_timing'):
            run_cub_baseline._timing = {}
        run_cub_baseline._timing['kernel_ms'] = kernel_ms
        run_cub_baseline._timing['kernel_time_s'] = kernel_ms / 1000.0
        run_cub_baseline._timing['energy_j'] = energy_j
        run_cub_baseline._timing['power_w'] = power_w
...
def measure_once_gpu(N, kernel_func, target_runtime_s, args=None):
    ...
    if args and args.impl == 'cupy':
        ...
        result = run_cub_baseline(x, out, args)
        # Use timing from CUB binary (CUDA Events) for accurate N-scaling
        if hasattr(run_cub_baseline, '_timing'):
            kernel_ms = run_cub_baseline._timing['kernel_ms']
            kernel_time_s = run_cub_baseline._timing['kernel_time_s']
    else:
        kernel_func(x, out, passes)
        result = float(out[0])
    ...
    # Calculate timing (use CUB binary timing if available, otherwise CUDA Events)
    if not (args and args.impl == 'cub_cpp' and hasattr(run_cub_baseline, '_timing')):
...
def main():
    ...
    if device == 'GPU':
        ...
        if args.impl == 'cupy':
            ...
            # Use timing and energy from CUB binary (CUDA Events + NVML)
            if hasattr(run_cub_baseline, '_timing'):
                kernel_ms = run_cub_baseline._timing['kernel_ms']
                kernel_time_s = run_cub_baseline._timing['kernel_time_s']
        else:  # custom
        ...
        if args.impl != 'cub_cpp':
            # For non-CUB implementations, use CUDA Events from Python
            kernel_ms = cp.cuda.get_elapsed_time(start_event, end_event)
            kernel_time_s = kernel_ms / 1000.0
        # For CUB, kernel_ms and kernel_time_s are already set above
    ...
    # For CUB implementation, override with binary's own NVML measurements
    if device == 'GPU' and args.impl == 'cub_cpp' and hasattr(run_cub_baseline, '_timing'):
        if run_cub_baseline._timing.get('energy_j') is not None:
            energy_gpu = run_cub_baseline._timing['energy_j']
            # Also adjust energy window to match kernel timing for consistency
            energy_window_s = kernel_time_s
    ...
    # For CUB implementation, override power calculation with binary's direct calculation
    if device == 'GPU' and args.impl == 'cub_cpp' and hasattr(run_cub_baseline, '_timing'):
        if run_cub_baseline._timing.get('power_w') is not None:
            power_gpu = run_cub_baseline._timing['power_w']
    ...
    if args.impl == 'cub_cpp' and hasattr(run_cub_baseline, '_timing'):
        if 'energy_J' in run_cub_baseline._timing and 'power_W' in run_cub_baseline._timing:
            energy_j = run_cub_baseline._timing['energy_J']
            power_w  = run_cub_baseline._timing['power_W']
